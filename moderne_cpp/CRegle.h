...
public:
	...
	virtual void REG_CalculerPointsManche(unique_ptr<CPaquet>& pPli, unsigned int uiIndiceJoueurGagnant, map<unique_ptr<CEquipe>, int>& mPoints, unique_ptr<CPaquet>& pDefausse, const vector<unique_ptr<CJoueur>>& vuJoueurs) = 0;

	// Mise en place

	virtual bool REG_VerificationNbJoueur(unsigned int uiNbJoueurs) = 0;
	virtual void REG_ConstituerEquipes(vector<unique_ptr<CJoueur>>& vuJoueurs, map<unique_ptr<CEquipe>, int>& muPointsEquipe, unsigned int uiIndincePreneur = 0) = 0;
	...
	virtual void REG_MettreEnPlacePioche(unique_ptr<CPaquet>& paquet, vector<unique_ptr<CJoueur>>& joueurs) = 0;
